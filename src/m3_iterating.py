...
def run_test_count_negatives():
    ...
    # Test 6:
    expected = 0
    actual = count_negatives(([0, 0, 2, 1, 9, 8, 0, 2, 4, 50, 102, 39503]))
    print()
    ...
    # Test 7:
    expected = 5
    actual = count_negatives(([0, -2, 9, -80, -450, -1023, -9503]))
    print()
...
def run_test_count_short_ones():
    ...
    # Test 8:
    expected = 2
    seq = [[], [1000, "Yes", "no"], ["We", "Are", "the", "Champions"], [11, 1]]
    actual = count_short_ones(seq)
    print()
...
def draw_circles(window, points, radius, color):
    ...
    # ------------------------------------------------------------------
    for k in points:
        kite = rg.Circle(k, radius)
        kite.fill_color = color
        kite.attach_to(window)
        window.render(0.05)
